...
def start_hotspot():
    ...
    #does *something* 
    subprocess.run(['sudo','nmcli', 'connection', 'modify', hotspot_name,'802-11-wireless.mode', 'ap','ipv4.method', 'manual', 'ipv6.method', 'ignore'], check=True)
    ...
    subprocess.run(['sudo','nmcli', 'connection', 'modify', hotspot_name,'ipv4.gateway', '10.42.0.1','ipv4.dns', '10.42.0.1'], check=True)
    #just starts it up
